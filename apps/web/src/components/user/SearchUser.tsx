...
export default function SearchUser({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
  ...
  const onUserSelect = (user: User) => {
    if (selectedUsers.map((u) => u.id).includes(user.id)) {
      return setSelectedUsers(selectedUsers.filter((selectedUser) => selectedUser.id !== user.id));
    }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="gap-0 p-0 outline-none">
        ...
        <Command className="overflow-hidden rounded-t-none border-t bg-transparent" shouldFilter={false}>
          <CommandInput placeholder="Search user by name or email..." value={query} onValueChange={onChange} />
          <CommandList>
        ...
        <DialogFooter className="flex items-center border-t p-4 sm:justify-between">
          ...
          <Button disabled={selectedUsers.length < 1} onClick={createNewConversation}>
            Continue
          </Button>
        </DialogFooter>
      </DialogContent>
